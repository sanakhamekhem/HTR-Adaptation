...
def spatial_transform(x, Scope, imageHeight, imageWidth):  
	## x shape: [N, W, H, C=1]
	...
	with tf.variable_scope(Scope):
		...
		n_fc = 6
		W_fc1 = tf.Variable(tf.zeros([imageHeight * imageWidth * 1, n_fc]), name='W_fc1')
		...
		h_fc1 = tf.matmul(tf.zeros([1, imageHeight * imageWidth  * 1]), W_fc1) + b_fc1
		
		 
		imageHeight=int(imageHeight/2)
		imageWidth=int(imageWidth/2)
		stn = st.transformer(x, h_fc1, (imageHeight, imageWidth))
...
def target_list_to_sparse_tensor(targetList):
    ...
    # print('indexxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
    # print(np.asarray(indices))
    indices = np.asarray(indices, dtype=np.int64)
    vals = np.asarray(vals, dtype=np.int32)
    shape = np.asarray([len(targetList), np.asarray(indices).max(0)[1] + 1], dtype=np.int64)
    # shape = [len(targetList), np.asarray(indices).max(0)[1] + 1]
    # return (np.array(indices), np.array(vals), np.array(shape))
    return (indices, vals, shape)


def binarize_image(image_path, im_name, database):
    import numpy as np 
    ...
    blur = cv2.GaussianBlur(rect, (5, 5), 0)
    ret3, thresh_img = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    cv2.imwrite('/home/ahmed/Desktop/sana/RCNN/HTRTF/converted' + database + '/' + im_name, thresh_img) 

def load_dataset(h5_file, DataBasePath, log_file_indicator, x_pad=0, y_pad=0, normalization=True, resize=True, padding=False, database='IAM'):
    """
    ...
    try:
        ...
        init_time = time()
        imageHeight =70# max(fh5['data/sizes'][:, 1]) + y_pad * 2
        imageWidth = 2200  # #2000 ###max(fh5['data/sizes'][:, 0]) + x_pad * 2
        image_extension = 'tif'
        if database == 'KHATT':
            image_extension = 'tif'
            imageWidth = 2200
        ...
        for i in range(set_size):
            ...
            if database == 'IAMd':
                img_path = str.rstrip(DataBasePath, '/') + '/' + img_name_split[0] + '/' + img_name_split[0] + '-' + img_name_split[1] + '/' + img_name + '.' + image_extension
            else:
                img_path = str.rstrip(DataBasePath, '/') + '/' + img_name + '.' + image_extension
            try:
                img = Image.open(img_path)
                if database == 'AHTID':
                    binarize_image(img_path, img_name + '.' + image_extension, database) 
                    img = Image.open('/home/ahmed/Desktop/sana/RCNN/HTRTF/convertedAHTID/' + img_name + '.' + image_extension)
                elif database == 'MADCAT':
                    binarize_image(img_path, img_name + '.' + image_extension, database) 
                    img = Image.open('/home/ahmed/Desktop/sana/RCNN/HTRTF/convertedMADCAT/' + img_name + '.' + image_extension)    
                else:
                   img = Image.open(img_path)
                ...
                if resize:
                    ...
                    # img.save('/home/ahmed/Desktop/sana/RCNN/HTRTF/convertAHTID/' + img_name + '.png')
                    img_numpy = np.array(img.getdata()).reshape(imageHeight, imageWidth)                 
                    if padding:
                        ...
                        seqLengths.append(imageWidth + x_pad)
                        seqHeights.append(imageHeight + y_pad)
                    else:
                       seqLengths.append(imageWidth)
                       seqHeights.append(imageHeight) 
                else:
                    img_numpy = np.array(img.getdata()).reshape(imageHeight, imageWidth)
                    seqLengths.append(imageWidth)
                    seqHeights.append(imageHeight)
...
def main():
    ...
    log_path = '/home/ahmed/Desktop/sana/RCNN/HTRTF/Projects/AHTID/log_test.txt'
    
    h5_file_path = '/home/ahmed/Desktop/sana/RCNN/HTRTF/Projects/AHTID/Sets/list_test.h5'
    DataBasePath = '/home/ahmed/Desktop/sana/RCNN/HandwritingRecognitionSystem/sampleAHTID/Images/'
    
    log_file_indicator = initialize_log(log_path)
    
    load_dataset(h5_file_path, DataBasePath, log_file_indicator, database='AHTID')
