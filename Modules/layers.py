...
def max_pool(x, pool_size, seq_len, imageHeight, imageWidth, evalFLAG):
    ...
    seq_len = tf.cast(tf.ceil(tf.to_float(seq_len)/pool_size[1]),dtype=tf.int32) 
...
def max_pool_2x2(x, seq_len, imageHeight, imageWidth):
    return tf.nn.max_pool(x, ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME'), tf.cast(tf.ceil(tf.to_float(seq_len)/2),dtype=tf.int32), ceil(imageHeight/2), ceil(imageWidth/2)
...
def extract_2x2_patches(x,seq_len, imageHeight, imageWidth):
    ...
    seq_len = tf.cast(tf.ceil(tf.to_float(seq_len)/2),dtype=tf.int32) 
