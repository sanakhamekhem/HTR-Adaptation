...
def train(step, network, num_epochs, batchSize, learning_rate, trainSet, imageHeight, imageWidth, num_classes, log_indicator, models_path, train_writer, transferFLAG = False, dataAugmentation = False,stage):        
    ...
    graph, saver, inputs_mask_ph, seq_len_ph, targets_ph, targets_len_ph, learning_rate_ph,  n_batches_ph, setTotalChars_ph, previousEDabs_ph, previousEDnorm_ph, previousCost_ph, optimizer, batch_cost, cost, errors, ED, predictions, merged = network.create(imageHeight,imageWidth, num_classes, False,stage)
...
def validation(epoch, network, batchSize,set_name,Set, imageHeight,imageWidth,labels, num_classes,log_indicator, models_path, valid_writer, AACHEN_init=False, AACHEN_h5_file=[], dataAugmentation = False,stage):
...
def transfer(epoch, network, imageHeight, imageWidth, num_classes, log_indicator, original_models_path, new_models_path, train_writer,stage):

       graph, [saver, transfer_saver], inputs_mask_ph, seq_len_ph, targets_ph, targets_len_ph, learning_rate_ph, n_batches_ph, setTotalChars_ph, previousEDabs_ph, previousEDnorm_ph, previousCost_ph, optimizer, batch_cost, cost, errors, ED, predictions, merged = network.create(imageHeight,imageWidth, num_classes, False,stage)
